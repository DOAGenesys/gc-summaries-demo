...
interface SharedConversationGroupProps {
...
export function SharedConversationGroup({
    conversations,
    locale,
    translations,
    primaryColor,
}: SharedConversationGroupProps) {
    if (conversations.length === 0) return null

    return (
        <div className="relative space-y-3 pl-4">
            {/* Visual connector for shared parentship */}
            <div
                className="absolute left-0 top-6 bottom-6 w-1 rounded-full opacity-60"
                style={{
                    background: `linear-gradient(180deg, ${hexToRgba(primaryColor, 0.8)}, ${hexToRgba(primaryColor, 0.2)})`
                }}
            />
            {/* Small link icon to indicate connection */}
            <div
                className="absolute -left-1.5 top-1/2 -translate-y-1/2 size-4 rounded-full bg-white border shadow-sm flex items-center justify-center z-10"
                style={{ borderColor: primaryColor }}
            >
                <div className="size-1.5 rounded-full" style={{ background: primaryColor }} />
            </div>
